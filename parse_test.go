...
func TestParse(t *testing.T) {
	...
	for _, tc := range tt {
		...
		if len(cmds) != tc.wantCmdsLen {
			t.Errorf("TestParse failed on number of cmds: name %s, wanted %d, got %d", tc.name, tc.wantCmdsLen, len(cmds))
		}
		if len(expls) != tc.wantExplsLen {
			t.Errorf("TestParse failed on number of cmds: name %s, wanted %d, got %d", tc.name, tc.wantExplsLen, len(expls))
		}
		if len(nested) != tc.wantNestedLen {
			t.Errorf("TestParse failed on number of nested cmds: name %s, wanted %d, got %d", tc.name, tc.wantNestedLen, len(nested))
		}
		if !reflect.DeepEqual(cmds, tc.wantCmds) {
			t.Errorf("TestParse failed on cmd equality: name %s, wanted %q, got %q", tc.name, tc.wantCmds, cmds)
		}
		if !reflect.DeepEqual(expls, tc.wantExpls) {
			t.Errorf("TestParse failed on explanations equality: name %s, wanted %q, got %q", tc.name, tc.wantExpls, expls)
		}
		if !reflect.DeepEqual(nested, tc.wantNested) {
			t.Errorf("TestParse failed on nested equality: name %s, wanted %q, got %q", tc.name, tc.wantNested, nested)
		}
	}
}
